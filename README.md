# BankingAsk
Banking Rules, Law,Regulation 

https://drive.google.com/file/d/1pdRothacqdkC-j_3paabDw9g88Bn7NvL/view?usp=drivesdk


<h3>Elaborate / Elaboration </h3>


<ul style="list-style-type:circle;">

SACP= special agriculture credit program 
BCBS= Basel Committee on Banking Supervision
CRAR= capital to risk weighted asset ratio 
BIS= bank for international settlement 
APA= annual performance agreement 
RSIW= rescheduled interest remission 
PR= performance  register 
FMJ= foreign money and jewellery 
TM Form= Travel & miscellaneous form
RMA= relationship management application 

BRPD-Banking Regulation and Policy Department
MCD, RCD, 
MOU-Memorandum of Understanding
RS -Revision Settlement)
CBS- Core Banking System 
 SWIFT- The Society For Worldwide Financial Transaction 
 BEFTN- Bangladesh Electric Fund Transfer Network 



COC= CODE OF CONDUCT
DCC= DISCIPLINARY CASES COMMITTEE 
DAD= DISCIPLINE AND APEAL DIVISION
 LDCL,=loan Documentation Checklist 
BMRE=  BALANCING , MODERNIZATION, REHABILITATION, Expansion 
API= APPLICATION PROGRAMMING INTERFACE 
SBS= SCHEDULED BANK STATISTICS 
PDR= public Demand Recover 
ETRAN= Entry of Transaction, SACP=SPECIAL AGRICULTURE CREDIT PROGRAM
BLW= Bad or Loss Write off
CAR= Capital Adequacy Ratio
RWA= RISK WEIGHTED ASSESTS
MCR= MINIMUM CAPITAL REQUIREMENTS
OBS= OFF BALANCE SHEET
CCF= CREDIT CONVERSION FACTOR
SRP= SUPERVISORY REVIEW PROCESS
LCR= LIQUIDITY COVERAGE RATIO
NFSR= NET STABLE FUNDING RATIO
CUMED: Credit for Urban Women Micro Enterprise Development

ToR= TERMS OF REFERENCE 
ADR= ALTERNATIVE DISPUTE RESOULATION 

ADR= Alternative Dispute Resolution 
CAG= COMPTROLLER & AUDITOR GENERAL
CCR= CLEAR CERTIFICATE OF RECTIFICATION 
ICC= INTERNAL CONTROL & COMPLIANCE
DCFCL= DEPARTMENTAL CONTROL FUNCTION CHECKLIST 
QOR= QUARTERLY OPERATION REPORT
SMA = SPECIAL MENTION ACCOUNT 
TC= TRAVELER'S CHEQUE





তৃতীয় পরিক্ষা। 


csme scheme ঃ
Women Enterprise Scheme, Re- Financing Scheme, Pre-Financing Scheme, Cluster Scheme, Start-Up Scheme, Credit Guarantee Scheme, General CMSME Scheme


CMSME = Cottage, Micro, Small and Medium Enterprises
ICC = International Chamber of Commerce
CCI&E = Chief Controller of Imports & Exports
FERA = Foreign Exchange Regulation Act
GFET = Guidelines for Foreign Exchange Transactions
UCPDC = Uniform Customs and Practice for Documentary Credits
ISBP = International Standard Banking Practice
URR = Uniform Rules for Reimbursements
URC = Uniform Rules for Collections
IRC = Import Registration Certificate
ERC = Export Registration Certificate
BTBLC = Back-to-Back Letter of Credit

NPV = Net Present Value
IRR = Internal Rate of Return
DSCR = Debt Service Coverage Ratio
PBP = Payback Period
IDCP = IDCP=Interest During Construction Period
CFR = Cost & FEIGHT
ROA = Return on Assets

FRTB= Floating Rate Treasury Bond
TMD = Treasury Management Department
 ALS= Assured Liquidity Support
ALCO = Asset Liability Committee
WEDD = Women Entrepreneurship Development Desk
3R = Reduce, Reuse, Recycle
SDG = Sustainable Development Goals
MDG = Millennium Development Goals
ESDD = Environmental & Social Due Diligence
ETP = Effluent Treatment Plant

IDCP=Interest During Construction Period

FRTB= Floating Rate Treasury Bond
ALS= Assured Liquidity Support

</ul>

